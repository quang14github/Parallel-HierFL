...
from datasets.prepare_data import get_gquic
...
class MTL_Model(object):
    ...
    def __init__(
        ...
        self.optimizer = optim.SGD(params=param_dict, lr=learning_rate)
...
def main():
    ...
    # )
    trainset, testset = get_gquic(dataset_root="data", args=args)
    trainloader = torch.utils.data.DataLoader(
        trainset, batch_size=128, shuffle=True, num_workers=2
    )
    testloader = torch.utils.data.DataLoader(
