...
def split_data(dataset, args, kwargs, is_shuffle=True):
    ...
    if args.iid == 1:
        ...
        raise ValueError(
            "Data Distribution pattern `{}` not implemented ".format(args.iid)
        )
    return data_loaders
...
def get_dataset(dataset_root, dataset, args):
    ...
    if dataset == "mnist":
        train_loaders, test_loaders, v_train_loader, v_test_loader = get_mnist(
            dataset_root, args
        )
    elif dataset == "cifar10":
        train_loaders, test_loaders, v_train_loader, v_test_loader = get_cifar10(
            dataset_root, args
        )
...
def get_mnist(dataset_root, args):
    ...
    v_train_loader = DataLoader(
        train, batch_size=args.batch_size * args.num_clients, shuffle=True, **kwargs
    )
    v_test_loader = DataLoader(
        test, batch_size=args.batch_size * args.num_clients, shuffle=False, **kwargs
    )
    return train_loaders, test_loaders, v_train_loader, v_test_loader
...
if __name__ == "__main__":
    ...
    train_loaders, test_loaders, _, _ = get_dataset(
        args.dataset_root, args.dataset, args
    )
    print(
